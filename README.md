...
Do you have a big vault (3k+ files)? Have you tried to use mobile Obsidian to open it? Then you know how awfuly long the loading takes. At some point the mobile app simply becomes unusable at all, not just for quick notes. Obsidian team have claimed that they have fixed that in 1.7, but this is just plain false as the app is still unbearable for anything larger that a few thousand files.
...
You store one note per concept. You have "Paris", "Eifel Tower", "Attractions", "France" and "Capitals". With folders you can start sorting everything like that: Capitals -> Paris -> Eifel Tower. So far so good. But why not Attractions -> Eifel Tower, why not France -> Paris -> Eifel Tower? Because you can not have a file in two folders at once, so you have to choose either one or another. This is not good, your only choice is to lose valuable information (that Eifel Tower is an attraction). Other "solutions" include duplicating folders (create Attractions folder for each city) which make searching harder and essentially lose information just in a more peculiar way.

Now lets try to use tags. We could go like this: #Attractions, #Paris, #France -> Eifel Tower. Have you spotted the problem? Now we have redundancy! This is also bad, as it is obvious that something in #Paris is also in #France, and we have to either add both by hand (and risk hesitation and/or human error) or lose information. And no, _nested tags_ are just duplicated tags in disguise. And see what problem we would have with them: #France/Paris -> Eifel Tower or #Capitals/Paris -> Eifel Tower? If only we could give #Paris several tags...

That's where ftags save the day! Now you can have arbitrary DAG as your tag-system. #Attractions, #Paris -> Eifel Tower; #France, #Capitals -> #Paris
...
Previously I modelled it with symlinks (which is neat and tidy), but, though a part of POSIX standard, a lot of software behaves poorly with symlinks. And it is totally not transportable to other OSes, even Android.
...
This plugin should not interfere with other plugins and should work transparently for both the user and other plugins, but as any plugin may monkey-patch anything anywhere arbitrarely something may break somewhere. Keep that it mind and see the previous paragraph.
...
Issues and patches are welcome. This plugin is intended to be used with other plugins and I would try to do my best to support this usecase, but I retain the right to refuse supporting any given plugin for arbitrary reasons.
